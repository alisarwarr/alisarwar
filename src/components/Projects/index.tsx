...
type ProjectsProps = {
    projects: Array<{id: number, name: string, technologies: string, url: string, pic: any}>;
...
function Projects({ projects, services }: ProjectsProps) {
    ...
    existProjects = [ ...projects.map(({ technologies }) => technologies.includes(filter)) ];
    ...
    var filterProjects: any[] = []; var i:number;
    for(i of indexes) {
    ...
    return (
        <Zoom duration={1300}>
            <div className={`d-flex justify-content-center align-items-center flex-column pl-7 pr-7 ${screen350?`mt-16`:`mt-12`} selection:bg-transparent`}>
                {
                    screen640?(
                        screen450?
                        ...
                        :
                            <Typography variant="h2" className="mt-16 fw-bold text-white bg-black cursor-default pt-2 pb-3 pl-8 pr-8 rounded-xl flex align-middle shadow-xl shadow-gray-700/75">
                                <span className="xs:mt-3 sm:mt-2.5 pr-2.5"> Projects </span>
                                <Image src={project} width="70rem" height="46.75rem" className="mt-1"/>
                            </Typography>
                    ):(
                        <Typography variant="h2" className="mt-16 fw-bold text-white bg-black cursor-default pt-2 pb-3 pl-8 pr-8 rounded-xl flex align-middle shadow-xl shadow-gray-700/75">
                            <span className="sm:mt-2.5 md:mt-2 lg:mt-1.5 xl:mt-1 2xl:mt-0.5 pr-2.5"> Projects </span>
                            <Image src={project} width="90rem" height="70rem" className="mt-1"/>
                        </Typography>
                    )
                }
